...
{
    public class CargoRepositorioPruebas
    {
        ...
        [Fact(DisplayName = "Repositorio_Cargo_CRUD")]
        public void Repositorio_Cargo_CRUD()
        {
            var opts = CrearOpciones("Cargodb" + Guid.NewGuid());

            using (var contexto = new FullBibliotecaContexto(opts))
            {
                var repo = new CargoRepositorio(contexto);

                var entidad = new Cargo { Nombre = "Prueba Cargo" };
                var creado = repo.Agregar(entidad);
                Assert.True(creado.Id != 0);

                var obtenido = repo.ObtenerPorId(creado.Id);
                Assert.NotNull(obtenido);
                Assert.Equal("Prueba Cargo", obtenido.Nombre);
