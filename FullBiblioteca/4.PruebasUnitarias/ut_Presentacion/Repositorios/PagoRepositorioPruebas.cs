...
{
    public class PagoRepositorioPruebas
    {
        ...
        [Fact(DisplayName = "Repositorio_Pago_CRUD")]
        public void Repositorio_Pago_CRUD()
        {
            var opts = CrearOpciones("Pagodb" + Guid.NewGuid());

            using (var contexto = new FullBibliotecaContexto(opts))
            {
                var repo = new PagoRepositorio(contexto);

                var entidad = new Pago { Nombre = "Prueba Pago" };
                var creado = repo.Agregar(entidad);
                Assert.True(creado.Id != 0);

                var obtenido = repo.ObtenerPorId(creado.Id);
                Assert.NotNull(obtenido);
                Assert.Equal("Prueba Pago", obtenido.Nombre);
