...
{
    public class PaisRepositorioPruebas
    {
        ...
        [Fact(DisplayName = "Repositorio_Pais_CRUD")]
        public void Repositorio_Pais_CRUD()
        {
            var opts = CrearOpciones("Paisdb" + Guid.NewGuid());

            using (var contexto = new FullBibliotecaContexto(opts))
            {
                var repo = new PaisRepositorio(contexto);

                var entidad = new Pais { Nombre = "Prueba Pais" };
                var creado = repo.Agregar(entidad);
                Assert.True(creado.Id != 0);

                var obtenido = repo.ObtenerPorId(creado.Id);
                Assert.NotNull(obtenido);
                Assert.Equal("Prueba Pais", obtenido.Nombre);
