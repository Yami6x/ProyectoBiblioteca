...
{
    public class EditorialRepositorioPruebas
    {
        ...
        [Fact(DisplayName = "Repositorio_Editorial_CRUD")]
        public void Repositorio_Editorial_CRUD()
        {
            var opts = CrearOpciones("Editorialdb" + Guid.NewGuid());

            using (var contexto = new FullBibliotecaContexto(opts))
            {
                var repo = new EditorialRepositorio(contexto);

                var entidad = new Editorial { Nombre = "Prueba Editorial" };
                var creado = repo.Agregar(entidad);
                Assert.True(creado.Id != 0);

                var obtenido = repo.ObtenerPorId(creado.Id);
                Assert.NotNull(obtenido);
                Assert.Equal("Prueba Editorial", obtenido.Nombre);
